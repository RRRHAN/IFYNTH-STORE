...
interface TransactionCountCardProps {
  transactionCounts: TransactionCount[] | null;
  cardTitle?: string;
  isLoading?: boolean;
  hasError?: boolean;
...
const TransactionCountCard: React.FC<TransactionCountCardProps> = ({
  transactionCounts,
  cardTitle = "Transaction Counts by Status",
  isLoading = false,
  hasError = false,
  variant = 'dark'
}) => {
  ...
  if (isLoading) {
    return (
      ...
      >
        <ActivityIndicator size="large" color={textColorClass.includes('white') ? "#FFFFFF" : "#000000"} />
      ...
      {transactionCounts && transactionCounts.length > 0 ? (
        <ScrollView style={{ flexGrow: 1 }}>
          <Table className="w-full">
            ...
            <TableBody>
              {transactionCounts.map((data, index) => (
                <TableRow
                  key={data.Status}
                  className={index % 2 === 0 ? tableRowEvenClass : tableRowOddClass}
                >
                  <TableData className={`py-2 px-3 text-left`}>
                      <Badge
                        size="lg"
                        action={getBadgeAction(data.Status)}
                        className="w-fit justify-center"
                        style={{width: 200}}
                      >
                        <BadgeText>{data.Status.charAt(0).toUpperCase() + data.Status.slice(1)}</BadgeText>
                      </Badge>
                  </TableData>
                  <TableData className={`py-2 px-3 text-right ${tableContentTextColorClass}`}>
